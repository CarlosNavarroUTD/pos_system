...
const ListaClientes = () => {
  ...
  const cargarClientes = async () => {
    try {
      ...
      let url = 'http://localhost:8000/api/ventas/clientes/';
      if (busqueda) {
        url += `?q=${busqueda}`;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = clienteEditando 
        ? `http://localhost:8000/api/ventas/clientes/${clienteEditando.id_cliente}/`
  ...
  const eliminarCliente = async (id) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:8000/api/ventas/clientes/${id}/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
